...
17. [Group-Target Set 问题](#17-group-target-set-问题)
18. [vLLM 长短分流路由（vllm-router.py）](#18-vllm-长短分流路由vllm-routerpy)
19. [CI 时区相关测试失败](#19-ci-时区相关测试失败)
...
## 18. vLLM 长短分流路由（vllm-router.py）

> `vllm-router.py` 是独立的 FastAPI 脚本，按估算 token 把请求分流到「短池」（高并发、承载中小请求）和「长池」（低并发、承载大上下文），并对长请求做会话亲和。以下为该脚本的实战排查与设计复盘。

### 18.1 上游 500：`httpx.RemoteProtocolError: Server disconnected without sending a response`

**症状**：客户端收到 `500 Internal Server Error`，路由日志打出整段 httpx traceback，traceback 停在 `_receive_response_headers`（即**收到响应头之前**连接就断了）。

**根因**（两类，需分别处理）：
1. **keep-alive 连接复用到已被服务端关闭的连接**（最常见）。全局共享 `httpx.AsyncClient` 默认开连接池，当 vLLM 侧的 idle keepalive 超时比客户端短，httpx 从池里取到一条服务端早已半关闭的连接，下一个 POST 立刻得到该错误。**重试一次即可成功**（会建立新连接）。
2. **上游 vLLM 真的拒绝/崩溃**（OOM、输入超 `max-model-len`、进程重启）。重试同一后端会继续失败，需靠健康检查摘除。

**修复**：
- 新增 `_send_upstream()`，对连接级错误（`RemoteProtocolError` / `ConnectError` / `ConnectTimeout` / `ReadError`）在**收到响应头之前**自动重试 `UPSTREAM_CONNECT_RETRIES`（默认 2）次。这类断点重试对 POST 是可接受的（服务端极大概率尚未处理请求）。
- 重试耗尽后返回干净的 `502 + JSON`，不再让 traceback 冒泡成裸 500。
- 用 `release()` 闭包统一所有返回路径的计数释放，避免漏释放导致计数泄漏。

**经验**：
- 「disconnect-before-headers」是连接池陈旧连接的典型特征，**默认先怀疑陈旧连接而非业务错误**，加断点重试通常立竿见影。
- 标准 httpx 不会自动重试 POST（怕重复执行）；只在「发出后、收到响应头前」这个窗口手动重试才安全。
- 排查时看 traceback 停在哪一层：停在 `_receive_response_headers` → 还没拿到响应（陈旧连接/上游拒收）；停在流式读取中途 → 上游处理途中崩了（OOM 等），重试无用，去 vLLM 侧看日志。

### 18.2 中文请求 token 被严重低估，长请求被误路由到短池

**症状**：明显很长的中文对话被判为「短请求」送进短池，触发 18.1 第 2 类的上游断连或超 `max-model-len` 报错。

**根因**：路由用的 `estimate_tokens` 用固定系数 `chars * 0.3`（约 3.3 字符/token），按**英文/代码**校准；而中文约 1~1.5 字符/token。对中文按 0.3 估算只到真实值的约 1/3，导致大请求被误判为小请求。且该估算与 `/v1/messages/count_tokens` 端点各算各的，口径不一致。

**修复**：抽出 CJK 感知的公共函数 `chars_to_tokens()`（按中文字符占比把字符/token 系数在 2~4 之间动态调整），`estimate_tokens()`（路由）与 `count_tokens` 端点共用同一套逻辑。tools 序列化改用 `json.dumps(..., ensure_ascii=False)`，让中文 schema 按真实字符计入而非 `\uXXXX` 转义。

**经验**：token 估算必须区分语种——固定「字符×系数」对混合中英文一定会偏。一处估算逻辑（路由分流）和另一处对外口径（count_tokens）务必复用同一函数，否则两边漂移会埋下误路由的雷。

### 18.3 短池本应是「备用池」，却在长池空闲时被占用，拖慢短请求

**症状**：短池没有短请求时，长请求按「最少活跃长连接」选实例，短池空着（0）反而最容易被选中。结果长池并不忙时也往短池塞长请求；一旦短请求到来，要排在长请求后面，响应延迟飙升——违背了「短池只是备用」的初衷。

**根因**：`min(候选, key=long_active_conns)` 只看绝对负载，空闲的短池天然是最小值，没有体现「短池应优先让位给长池」的语义。

**修复（关键技巧：虚拟基线偏置）**：选实例时给短池节点的等效负载**叠加一个等于溢出容量的虚拟基线**（`_long_load()` 里 `+MAX_LONG_OVERFLOW`，即 +3）。这样长池每个节点的活跃长请求数没追平基线（3）之前，短池等效负载始终更高 → 长请求优先留长池；只有长池真正繁忙（≥3）才开始溢出到短池。短池真实容量仍是 3，叠加基线后等效负载范围 3→6，即「默认 3、最大 6」。
- **偏置只用于比较、绝不写入 `long_active_conns` 计数器**，所以日志里 `溢出` / `长请求活跃` 打印的永远是真实数。
- 同时：短池一旦有短请求（`short_pool_short_conns != 0`），长请求一律回长池——会话亲和复用短池节点的分支也加了这道门控，不管有没有会话缓存都不占用短池。
- 会话被迫迁移到长池时会刷新亲和表绑定到新长池节点，此后稳定留在长池，避免在两池之间反复横跳导致 prefix cache 抖动。

**经验**：要让某个资源池表现为「备用」而非「均摊」，不要在调度里给它真实计数加偏移（会污染监控/计数），而是**只在比较排序时叠加一个虚拟基线**——既改变了被选中的优先级，又保持了存储计数的真实性。这是一个可复用的「软优先级」实现范式。

---

## 19. CI 时区相关测试失败

**症状**：GitHub Actions 的 UTC runner 中，`TestTracker_SaveAllRequest` 读取 `track/all/YYYY-MM-DD_HH` 时提示目录不存在；同一测试在 UTC+8 开发机通过。

**根因**：`SaveAllRequest` 的既定语义是使用 `createdAt.Local()` 按运行环境本地时区归档，但测试把 UTC+8 的输入时间和 UTC+8 的目录名写死，隐含假定测试进程也在 UTC+8。

**修复与规则**：涉及 `time.Local()` 的测试必须从同一输入时间计算期望值（如 `createdAt.Local().Format(...)`），不能写死某一时区的日期、小时或格式化文本；需要验证跨时区行为时，应显式设置 `TZ` 并覆盖目标时区。
