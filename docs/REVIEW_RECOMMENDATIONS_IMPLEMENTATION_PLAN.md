...
> ## ⛔ 实施决策（2026-03-10）
>
> 以下四项**明确决定不实现**，原因见各节说明。任何 AI 助手或开发者请勿自动实施这些功能：
>
> | 编号 | 标题 | 决定 |
> |------|------|------|
> | Critical #1 Phase 1 | 对话追踪文件权限 0644 → 0600 | ⛔ 不实现 |
> | Critical #1 Phase 2/3 | 对话追踪加密 + 自动清理 | ⛔ 不实现 |
> | Critical #3 | JWT 黑名单持久化到 SQLite | ⛔ 不实现 |
> | Important #4 | 配额 fail_closed 配置项 | ⛔ 不实现 |
> | Quick Win #10 | 创建 docs/GOTCHAS.md | ⛔ 不实现 |
...
> **⛔ 决定：不实现（2026-03-10）**
> 当前部署场景为受控内网环境，服务器访问权限已受严格管控。文件权限修改、AES 加密及自动清理功能暂不纳入实施范围。如需合规支持，请评估后单独决策。
...
> **⛔ 决定：不实现（2026-03-10）**
> Token TTL 为 24 小时，重启后的风险窗口可接受。当前 sproxy 属于稳定常驻进程，重启频率极低。SQLite 持久化带来的复杂度与实际风险不成比例，暂不实施。
...
> **⛔ 决定：不实现（2026-03-10）**
> 当前业务优先可用性而非强制限额。数据库偶发故障时允许请求通过是可接受的权衡。如未来部署场景需要严格成本控制，可重新评估。
...
> **⛔ 决定：不实现（2026-03-10）**
> 相关陷阱内容已散落在 `docs/manual.md`、`docs/TROUBLESHOOTING.md`、`docs/SECURITY.md` 中，无需单独创建 GOTCHAS.md。
