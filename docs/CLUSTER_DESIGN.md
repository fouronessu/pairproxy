...
| **Config sync LLM target upsert** | LLM targets are upserted using `ON CONFLICT(url)` (not `ON CONFLICT(id)`), because `url` is the business-unique key. Worker and Primary independently generate UUIDs for the same config-file targets at startup, so `id` values diverge across nodes. The `url`-based conflict key ensures that syncing a target from Primary correctly updates the Worker's existing record (including overwriting the Worker's `id` with Primary's `id`) rather than failing with a UNIQUE constraint violation. Fixed in v2.14.1. |
