...
func (t *Tracker) SaveAllRequest(requestID, sessionID, userID, requestPath string, requestBody []byte, createdAt time.Time) error {
	req := json.RawMessage(`{}`)
	...
	createdAt = createdAt.Local().Truncate(time.Millisecond)

	data, err := json.Marshal(AllRequestRecord{
		CreatedAt:   createdAt.Format("2006-01-02_15:04:05.000"),
		SessionID:   sessionID,
		UserID:      userID,
	...
	hourDir := filepath.Join(allDir(t.dir), createdAt.Format("2006-01-02_15"))
	if err := os.MkdirAll(hourDir, 0o755); err != nil {
		return err
	}
	ts := createdAt.Format("2006-01-02_15-04-05")
	if requestID == "" {
		requestID = "unknown"
	}
	path := filepath.Join(hourDir, fmt.Sprintf("%s_%s.json", ts, requestID))
